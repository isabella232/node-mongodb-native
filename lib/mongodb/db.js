...
var QueryCommand = require('./commands/query_command').QueryCommand,
  ...
  debug = require('debug')('mongodb.db');
    ...
    var connectFunction = function(__server) {      
      ...
      new Db(object.dbName, __server, {safe:false, native_parser:false}).open(function(err, db) {
          ...
          if(!err) {
            ...
            if (object.db_options.standalone) {
              debug("standalone mode enabled")
              for (var key in isMasterDoc) {
                // strip out properties that describe the replica set
                if (key !== "ismaster" && key !== "maxBsonObjectSize" &&
                    key !== "localTime" && key !== "ok") {
                  delete isMasterDoc[key];
                }
              }

              // pretend the server we're connecting to is a master
