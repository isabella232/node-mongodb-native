var QueryCommand = require('./commands/query_command').QueryCommand,
  ...
  utils = require('./utils'),
  debug = require('debug')('mongodb.cursor');
...
function Cursor(db, collection, selector, fields, options) {
  ...
  this.oplogreplay = options.oplogreplay || false;
...
var generateQueryCommand = function(self) {
  ...
  if (self.oplogreplay) {
    /* yes, "REPLAY" is misspelled in query_command.js, so we just deal with it
       here to minimize diffs */
    queryOptions |= QueryCommand.OPTS_OPLOG_REPLY;
    debug("oplogreplay enabled");
  }
  ...
  if(self.state == Cursor.INIT) {
    ...
    if(self.connection == null) {
      try {
        ...
        if (self.connection && self.connection.socketOptions) {
          var so = self.connection.socketOptions;
          debug("cursor using connection to "+so.host+":"+so.port);
        }
      } catch(err) {
        return callback(utils.toError(err), null);
      }
